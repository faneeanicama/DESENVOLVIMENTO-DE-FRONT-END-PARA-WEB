...
imageFiles.forEach(file => {
  ...
  if (ext === '.jpg' || ext === '.jpeg') {
    reductionEstimate = 0.30; // 30% para JPG
  } else if (ext === '.png') {
    reductionEstimate = 0.20; // 20% para PNG
  }
  ...
  console.log(`    Estimado após compressão: ${(estimatedCompressed / 1024).toFixed(2)} KB`);
  console.log(`    Redução estimada: ${(reductionEstimate * 100).toFixed(1)}%\n`);
});

console.log('═══════════════════════════════════════');
console.log(`Total original estimado: ${(totalOriginal / 1024).toFixed(2)} KB`);
console.log(`Total comprimido estimado: ${(totalCompressed / 1024).toFixed(2)} KB`);
console.log(`Redução total estimada: ${((1 - totalCompressed / totalOriginal) * 100).toFixed(1)}%`);
console.log('═══════════════════════════════════════\n');
...
console.log('   E execute: npx imagemin Imagens/* --out-dir=Imagens --plugin=mozjpeg --plugin=pngquant');
